...
function App() {
    ...
    useEffect(() => {
        ...
        const savedDailyForecast = localStorage.getItem('dailyForecast');
        const savedHourlyForecast = localStorage.getItem('hourlyForecast');

        if (savedWeatherData && savedDailyForecast && savedHourlyForecast) {
           
            setWeatherData(JSON.parse(savedWeatherData));
            setDailyForecast(JSON.parse(savedDailyForecast));
            setHourlyForecast(JSON.parse(savedHourlyForecast));
        } else {
    ...
    const handleSearch = async () => {
        try {
            ...
            const weatherResponse = await Axios.get(`${api.base}/weather?q=${city}&units=metric&appid=${api.key}`);
            const weatherData = {
                description: weatherResponse.data.weather[0].description,
                temp: weatherResponse.data.main.temp,
                humidity: weatherResponse.data.main.humidity,
            ...
            const forecastResponse = await Axios.get(`${api.base}/forecast?q=${city}&units=metric&appid=${api.key}`);
            const forecastList = forecastResponse.data.list;
    ...
    const handleSaveLocation = async () => {
        if (city && !savedLocations.find(loc => loc.city === city)) {
            try {
                const response = await Axios.get(`${api.base}/weather?q=${city}&units=metric&appid=${api.key}`);
                const { coord } = response.data;
                setSavedLocations([...savedLocations, { city, lat: coord.lat, lon: coord.lon }]);
                setCity("");
            } catch (error) {
                    ...
                    <div className='hourly-list'>
                        {hourlyForecast.map((hour, index) => (
                            <div key={index} className='hour'>
                                ...
                                <p>Description: {hour.weather.description}</p>
                                <img src={`https://openweathermap.org/img/wn/${hour.weather.icon}.png`} alt="Hourly weather icon" />
                            </div>
                        ))}
                    </div>
                </div>
